...
SELECT id, 'publisher_benefits', JSON_OBJECT(
    'title', 'Why Publish With ClickNet',
    'items', JSON_ARRAY(
        ...
        JSON_OBJECT('title', 'Real-Time Reporting', 'description', 'Track clicks, conversions and earnings live from your dashboard.')
    )
...
SELECT id, 'advertiser_benefits', JSON_OBJECT(
    'title', 'Why Advertise With ClickNet',
    'items', JSON_ARRAY(
        ...
        JSON_OBJECT('title', 'Fraud-Free Traffic', 'description', 'Every click and conversion is checked by our fraud detection engine.')
    )
...
SELECT id, 'technology', JSON_OBJECT(
    ...
    'items', JSON_ARRAY(
        ...
        JSON_OBJECT('title', 'Postback & S2S', 'description', 'Seamless server-to-server integrations with all major platforms.'),
        JSON_OBJECT('title', 'Custom Dashboards', 'description', 'Purpose-built reporting dashboards for advertisers and publishers.'),
        JSON_OBJECT('title', 'Automated Optimization', 'description', 'Rules-based automation that reallocates spend to what converts.')
    )
...
SELECT id, 'content_block', JSON_OBJECT(
    ...
    'align', 'left',
    'body', 'Last updated: [date]\n\nBy accessing or using the ClickNet website and services, you agree to these Terms of Service.\n\nUse of Service: ClickNet operates a performance affiliate network connecting advertisers and publishers under CPS, CPL and CPI commercial models. Access to advertiser or publisher accounts is subject to a separate partner agreement.\n\nAcceptable Use: you agree not to use fraudulent, deceptive or automated means to generate clicks, leads, installs or sales.\n\nIntellectual Property: all content on this website is owned by ClickNet or its licensors and may not be reproduced without permission.\n\nLimitation of Liability: this website and its content are provided "as is" without warranties of any kind.\n\nThis is placeholder policy text. Replace it with counsel-reviewed language specific to your jurisdiction before launch.'
...
SELECT
    ...
    'How a mid-market fashion retailer grew profitable order volume through ClickNet''s publisher network.',
    'The brand came to ClickNet looking to diversify customer acquisition beyond paid search. Within 90 days of launch, our team onboarded 40+ vetted content and coupon publishers, implemented fraud-checked S2S tracking, and optimized commission tiers weekly based on live conversion data.\n\nThe result was a 160% increase in monthly order volume attributed to the affiliate channel, with a stable blended CPA and a 99.1% clean-traffic rate.',
    JSON_ARRAY(
